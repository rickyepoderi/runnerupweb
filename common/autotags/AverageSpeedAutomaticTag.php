...
/**
 * Simple AverageSpeedAutomaticTag tag.
 *
 * @author ricky
 */
class AverageSpeedAutomaticTag implements AutomaticTag {
    ...
    public function generateTagConfigWithExtra(Activity $activity): TagConfig {
        ...
        $tagConfig->setDescription('average speed limit tag');
        $tagConfig->setProvider(__CLASS__);
        $avgSpeed = floatval($activity->getDistanceMeters()) / floatval($activity->getTotalTimeSeconds());
        $tagConfig->setExtra($this->generateExtra($avgSpeed, 100.0));
        return $tagConfig;
    }
    ...
    public function isAssignable(Activity $activity, TagConfig $tagConfig): bool {
        ...
        $avgSpeedAct = floatval($activity->getDistanceMeters()) / floatval($activity->getTotalTimeSeconds());
        return floatval($config['greater']) <= $avgSpeedAct && $avgSpeedAct < floatval($config['less']);
    }
}
